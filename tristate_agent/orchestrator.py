...
v0.2 additions:
  - Micro-wake handoff routing (process_handoffs).
  - Milestone aggregation (all_milestones).
  - Phase-aware profile resolution via get_phase_profile().
  - Support for PhaseAwareAgent / specialized agent registration.
...
from .domain_profiles import get_domain_profile, get_phase_profile, DEFAULT_DOMAIN
...
class TristateOrchestrator:
  ...
  v0.2: Also manages micro-wake cross-branch handoffs between
  PhaseAwareAgent instances (ResearchAgent -> WritingAgent -> ReviewAgent).
  """
  ...
  def __init__(self, domain: str = DEFAULT_DOMAIN) -> None:
    ...
    # v0.2: handoff queue for micro-wake routing
  ...
  def _spawn_new_agent(
    ...
    **agent_kwargs: Any,
    ...
    """Create a new AgentNode (or subclass) and make it active."""
    current = self._active_agent()
    current.sleep()
    aid = self._new_agent_id()
    node = agent_cls(agent_id=aid, topic_label=topic_label, **agent_kwargs)
    self.agents[aid] = node
    self.agent_order.append(aid)
    self.active_agent_id = aid
    self.total_agents_spawned += 1
  ...
  # ------------------------------------------------------------------
  def _try_wake_dormant(
    self, message: str, embedding: Optional[list]
  ) -> Optional[str]:
    """
  ...
  # ------------------------------------------------------------------
  def _reabsorb_detour(
    self, agent: AgentNode, detour_turns: List[dict]
  ) -> None:
    """
  ...
  # ------------------------------------------------------------------
  # v0.2: Micro-wake handoff routing
  # ------------------------------------------------------------------
  def register_agent(
    self,
    agent: AgentNode,
    *,
    ...
    """Register a pre-constructed agent (e.g. PhaseAwareAgent subclass)."""
    if agent.agent_id in self.agents:
      return  # idempotent
    if make_active and self.active_agent_id is not None:
      self._active_agent().sleep()
    self.agents[agent.agent_id] = agent
    self.agent_order.append(agent.agent_id)
    self.total_agents_spawned += 1
    if make_active:
      self.active_agent_id = agent.agent_id

  def process_handoffs(self) -> List[Dict[str, Any]]:
    """Drain pending handoff requests from all PhaseAwareAgents.

    For each request whose target is a registered agent, wake that agent
    and make it active.  Returns list of processed handoff records.
    """
    # Collect handoffs from all agents that support drain_handoffs()
    for aid in list(self.agent_order):
      agent = self.agents[aid]
      if hasattr(agent, "drain_handoffs"):
        self._pending_handoffs.extend(agent.drain_handoffs())

    processed: List[Dict[str, Any]] = []
    for handoff in self._pending_handoffs:
      target_id = handoff.get("to")
      if target_id and target_id in self.agents:
        # Sleep current active, wake target
        if self.active_agent_id and self.active_agent_id != target_id:
          self._active_agent().sleep()
        target_agent = self.agents[target_id]
        target_agent.wake()
        self.active_agent_id = target_id
        processed.append(handoff)
    self._pending_handoffs.clear()
    return processed

  # ------------------------------------------------------------------
  # v0.2: Milestone aggregation
  # ------------------------------------------------------------------
  def all_milestones(self) -> List[Dict[str, Any]]:
    """Aggregate milestones across all registered PhaseAwareAgents.
    ...
    for aid in self.agent_order:
      agent = self.agents[aid]
      if hasattr(agent, "milestones"):
        for m in agent.milestones:
          milestones.append({"agent_id": aid, **m})
    milestones.sort(key=lambda m: m.get("timestamp", 0))
    return milestones
  ...
  def route(
    ...
    """
    # 0. Process any pending micro-wake handoffs first
  ...
  def registry_snapshot(self) -> List[dict]:
    ...
    for aid in self.agent_order:
      ...
      record = {
        "agent_id": aid,
        "topic_label": agent.topic_label,
        "state": self._agent_state_label(agent),
        "turn_count": agent.turn_count,
      }
      # v0.2: include phase and milestone count if available
      if hasattr(agent, "phase"):
        record["phase"] = agent.phase
      if hasattr(agent, "milestones"):
        record["milestone_count"] = len(agent.milestones)
      snap.append(record)
    return snap
